...
readonly SCRIPT_COMMANDS=(shell create slot slots revoke profiles projects profile info help -h --help add remove install allowlist clean save project tmux kill vpn-routing)
...
get_command_requirements() {
    ...
    case "$cmd" in
        ...
        info|profile|add|remove|install|allowlist|save|vpn-routing)
            echo "image"
            ;;
